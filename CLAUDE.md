...
- **DB client** (`src/db/index.ts`): a lazy Proxy picking Neon HTTP or PGlite. The Neon HTTP driver has **no interactive transactions**, so application code must not use `db.transaction`. Schema lives in `src/db/schema.ts`; migrations in `drizzle/` are generated, not hand-written.
- **Selected project**: most pages (dashboard, calendar, `/taches`) act on a "selected project" remembered in a cookie; resolution rules are in `src/lib/current-project.ts` (pure, tested) with the server wrapper in `src/lib/selected-project.ts`.
