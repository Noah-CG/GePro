...
4. Créez les tables, puis au choix :
   ...
   ```
5. Lancez `npm run dev`. Les comptes suivants se créent ensuite depuis l'app (**menu du compte → Gérer les membres**). Un compte ne voit aucun projet tant qu'il n'y est pas invité (voir [Projets étanches, membres et invitations](#projets-étanches-membres-et-invitations)).
...
### Mettre à jour une base existante : isolation des projets (migration 0011)

La migration `0011_isolation_projets` ajoute les membres, les invitations et le propriétaire des projets, et rattache les données existantes, **sans rien supprimer** :
...
Seules restent communes à tous les projets : la liste **Projets** (ses projets et les invitations reçues) et la gestion des **comptes** (administrateurs).
...
- **Administration** (administrateurs de l'application) : comptes ;
