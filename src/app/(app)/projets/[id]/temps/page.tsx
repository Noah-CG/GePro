...
type Props = { params: Promise<{ id: string }>; searchParams: Promise<{ membre?: string; historique?: string }> };

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { project } = await loadProjectPage((await params).id);
  return { title: `Temps de travail · ${project.name}` };
}
 ...
 * et celui de l'équipe sur ce projet. `?membre=<id>` affiche le temps d'un autre membre du
 * projet, sur ce projet seulement (propriétaire et administrateurs du projet).
 */
export default async function TimePage({ params, searchParams }: Props) {
  const { project, user: me, role } = await loadProjectPage((await params).id);
  const { membre, historique } = await searchParams;
  const team = await getProjectMembers(project.id);
  const canManage = atLeast(role, "admin");
  const timeHref = (memberId?: string) => `/projets/${project.id}/temps${memberId && memberId !== me.id ? `?membre=${memberId}` : ""}`;

  const targetId = membre && isUuid(membre) ? membre : me.id;
  const other = targetId === me.id ? null : team.find((m) => m.id === targetId);
  if (targetId !== me.id && (!canManage || !other)) redirect(timeHref());
  ...
  const [work, sessions, byProject, teamWork] = await Promise.all([
    ...
    getProjectTeamWork(project.id, today),
  ]);
  ...
  const baseHref = timeHref(member.id);
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <PageHeader
        ...
        actions={canManage && team.length > 1 && <MemberPicker projectId={project.id} team={team} value={member.id} meId={me.id} />}
      />
                  ...
                  <li key={m.id} className="flex items-center gap-3 text-sm">
                    ...
                    <div className="min-w-0 flex-1">
                      {canManage || m.id === me.id ? (
                        <Link href={timeHref(m.id)} className="block truncate hover:text-accent hover:underline">
                          {m.name}
                        </Link>
                      ) : (
