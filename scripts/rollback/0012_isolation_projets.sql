-- Retour arrière de drizzle/0012_isolation_projets.sql (npm run db:isolation-rollback).
--
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE '0012_isolation_projets : pas appliquée, rien à défaire.';
    RETURN;
  END IF;

  ALTER TABLE "project_events" ALTER COLUMN "project_id" DROP NOT NULL;
  IF to_regclass('public.migration_0012_team_events') IS NOT NULL THEN
    DELETE FROM "project_events" e USING "migration_0012_team_events" m
    WHERE m."event_id" = e."id" AND m."event_id" <> m."source_event_id";
    UPDATE "project_events" e SET "project_id" = NULL FROM "migration_0012_team_events" m
    WHERE m."event_id" = e."id" AND m."event_id" = m."source_event_id";
    DROP TABLE "migration_0012_team_events";
  END IF;
