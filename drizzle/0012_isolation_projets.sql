...
--    gardée dans migration_0012_team_events pour le retour arrière.
--
-- Aperçu avant application : npm run db:isolation-preview
-- Retour arrière : scripts/rollback/0012_isolation_projets.sql
DO $migration$
DECLARE
  first_project uuid;
BEGIN
  IF EXISTS (
    ...
    RAISE NOTICE '0012_isolation_projets : déjà appliquée.';
    RETURN;
  END IF;
  ...
  CREATE TABLE "migration_0012_team_events" (
  ...
  IF EXISTS (SELECT 1 FROM "project_events" WHERE "project_id" IS NULL) THEN
    ...
    INSERT INTO "migration_0012_team_events" ("event_id", "source_event_id", "project_id")
    ...
    FROM "migration_0012_team_events" m JOIN "project_events" e ON e."id" = m."source_event_id";
